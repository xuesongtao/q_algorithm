...
func (this *TableMgr) LoadTable() {
	for i := 0; i < 9; i++ {
		name := fmt.Sprintf("%s/table_%d.tbl", tbl, i)
		this.m_tbl[i].load(name)
	}

	for i := 0; i < 9; i++ {
		name := fmt.Sprintf("%s/eye_table_%d.tbl", tbl, i)
...
func (this *TableMgr) LoadFengTable() {
	for i := 0; i < 9; i++ {
		name := fmt.Sprintf("%s/feng_table_%d.tbl", tbl, i)
		this.m_feng_tbl[i].load(name)
	}

	for i := 0; i < 9; i++ {
		name := fmt.Sprintf("%s/feng_eye_table_%d.tbl", tbl, i)
